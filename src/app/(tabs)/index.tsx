...
function MesaCard({ mesa, onPress }: { mesa: Mesa; onPress: () => void }) {
  ...
  return (
    <Pressable style={styles.mesa} onPress={onPress}>
      <View
        style={[styles.circulo, { borderColor: color, backgroundColor: ocupada ? `${color}1A` : c.surface }]}
      >
        <Text style={styles.circuloNombre} numberOfLines={1}>
          {mesa.nombre || `${mesa.id}`}
        </Text>
        {ocupada ? <Text style={styles.circuloTotal}>{fmtMoneda(mesa.total)}</Text> : null}
      </View>
      {ocupada && mesa.mozo ? (
        <Text style={styles.mesaPie} numberOfLines={1}>
          {mesa.mozo}
        </Text>
      ) : (
        <Text style={[styles.mesaPie, { color }]} numberOfLines={1}>
          {rotuloEstado(mesa.estado)}
        </Text>
      )}
    </Pressable>
  );
...
const crear = (c: Tema) =>
  StyleSheet.create({
    ...
    mesa: { width: '29%', alignItems: 'center', gap: 6 },
    circulo: {
      width: '100%',
      aspectRatio: 1,
      borderRadius: 999,
    ...
    circuloNombre: { fontSize: 15, fontWeight: '800', color: c.text, textAlign: 'center' },
    circuloTotal: { fontFamily: c.monoSemi, fontSize: 12, color: c.text },
    mesaPie: { fontSize: 11, fontWeight: '700', color: c.muted, textTransform: 'uppercase', letterSpacing: 0.3 },
