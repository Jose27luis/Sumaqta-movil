...
Fase 3b (emisión real) implementada vía endpoint mobile en pro8: `POST api/mobile/restaurant/charge` (`MobileController@storeRestaurantCharge`). Recibe `{table_id, document_type_id (01/03/80), customer_id?, payment_method_type_id?, items:[{item_id, quantity}]}`, resuelve la serie por defecto del establecimiento, reusa `storeDocumentSimple` (01/03) o `storeSaleNoteSimple` (80) —que ya emiten por Facturalo, descuentan insumos y asocian a caja— y cierra la mesa (`RestaurantItemOrderStatus` pending→paid + limpia `current_session_id`). La app llama a este endpoint desde `cobro.api` (`cobrarMesa`), detrás de `env.cobroHabilitado`.

Estado del backend pro8: el método y la ruta están escritos y php-fpm reiniciado, pero **sin commitear ni pushear** (cambios en producción). Validar primero en un tenant demo (poner `env.cobroHabilitado = true` en una build de prueba, emitir contra demo, confirmar comprobante + cierre + caja), y recién entonces commitear pro8 y activar la bandera.
